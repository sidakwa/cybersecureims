...
Deno.serve(async (req) => {
  ...
  if (authUsersError) {
    ...
    return new Response(JSON.stringify({ error: 'Failed to load auth users' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }

  const { data: profiles, error: profilesError } = await supabase.from('profiles').select('*');
  if (profilesError) {
    console.error('profiles fetch error:', profilesError);
    return new Response(JSON.stringify({ error: 'Failed to load profiles' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
