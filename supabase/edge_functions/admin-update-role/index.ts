...
Deno.serve(async (req) => {
  ...
  if (callerProfileError) {
    ...
    return new Response(JSON.stringify({ error: 'Failed to validate caller' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  if (callerUser.id === userId && newRole !== 'admin') {
    return new Response(JSON.stringify({ error: 'Admins cannot remove their own admin role' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  if (existingProfileError) {
    ...
    return new Response(JSON.stringify({ error: 'Failed to validate target user' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
